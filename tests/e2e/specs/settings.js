describe("Settings/Persistency Tests", () => {
  it("Can change class year", () => {
    ...
    // Open dropdown
    cy.getByDataCy("selectClassYear").parent().click();

    // Find and click the displayed option in the dropdown that says 'Junior'
    cy.get(".v-menu__content").contains("Junior").click();

    // Submit dialog, closing it
    cy.getByDataCy("submitRoadSettingsButton").click();
  ...
  it("Saves open/close semester states", () => {
    ...
    // Accept cookies
    cy.getByDataCy("acceptCookies").click();

    // Define semesters
    cy.getByDataCy("road_$defaultroad$__semester_0").as("priorCredit");

    cy.getByDataCy("road_$defaultroad$__semester_0_dropZone").as(
      "priorCreditPanel",
    );

    cy.getByDataCy("road_$defaultroad$__semester_1").as("freshmanFall");

    cy.getByDataCy("road_$defaultroad$__semester_1_dropZone").as(
      "freshmanFallPanel",
    );

    cy.getByDataCy("road_$defaultroad$__semester_7").as("juniorFall");

    cy.getByDataCy("road_$defaultroad$__semester_7_dropZone").as(
      "juniorFallPanel",
    );

    // Open and close some semesters
    cy.get("@priorCreditPanel").should("not.be.visible");

    cy.get("@priorCredit").click();

    cy.get("@priorCreditPanel").should("be.visible");

    cy.get("@freshmanFallPanel").should("be.visible");

    cy.get("@freshmanFall").click();

    cy.get("@freshmanFallPanel").should("not.be.visible");

    cy.get("@juniorFallPanel").should("be.visible");

    cy.get("@juniorFall").click();

    cy.get("@juniorFallPanel").should("not.be.visible");
