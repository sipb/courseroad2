...
describe("Road tests", () => {
  it("Adds, renames, and deletes roads", () => {
    ...
    cy.getByDataCy("addRoadButton").click();

    // Add road named "Test Road abc"
    cy.get(".v-dialog--active").within(() => {
      cy.getByDataCy("newRoadName").type("Test Road abc");

      cy.getByDataCy("createRoadButton").click();
    });
    ...
    cy.get(".v-dialog--active").within(() => {
      cy.getByDataCy("deleteRoadButton").click();
    });

    // Confirm deletion of road
    cy.get(".v-dialog--active").within(() => {
      cy.getByDataCy("deleteRoadConfirmButton").click();
    });
  ...
  it("Duplicates roads", () => {
    ...
    cy.getByDataCy("classSearchInput").type("3.45");

    cy.dragAndDrop(
      '[data-cy="classInSearch3_45"]',
    ...
    cy.get(".v-dialog--active").within(() => {
      cy.getByDataCy("newRoadName").type("Duplicate Test Road");

      cy.get('.v-input[data-cy="duplicateSwitch"]').click();
    ...
    cy.get(".v-menu__content.menuable__content__active").within(() => {
      cy.get(".v-list__tile").contains("My First Road").click();
    });
    ...
    cy.getByDataCyPattern("^", "roadTab")
      ...
      .as("roadID")
      .then(function () {
        cy.getByDataCy("roadTab" + this.roadID).click();
      });

    // Ensure 3.45 is in duplicated road
    cy.get("@roadID").then((roadID) => {
      cy.getByDataCy("road_" + roadID + "__semester_1").within(() => {
        cy.getByDataCy("classInSemester1_3_45").should("exist");
      });
    });

    // Go back to default road
    cy.getByDataCy("roadTab$defaultroad$").click();
    ...
    cy.getByDataCy("road_$defaultroad$__semester_3").within(() => {
      cy.getByDataCy("classInSemester3_7_46").should("exist");
    });

    // Check that 3.45 but NOT 7.46 is in the duplicated road
    cy.get("@roadID").then((roadID) => {
      cy.getByDataCy("roadTab" + roadID).click();

      cy.getByDataCy("road_" + roadID + "__semester_1").within(() => {
        cy.getByDataCy("classInSemester1_3_45").should("exist");
      });

      cy.getByDataCy("road_" + roadID + "__semester_3").within(() => {
        cy.getByDataCy("classInSemester3_7_46").should("not.exist");
      });
    });
  });
  ...
  it("Imports roads", () => {
    ...
    cy.route(
      "POST",
      Cypress.env("VUE_APP_FIREROAD_URL") + "/requirements/progress/master6-P/",
      {
    ...
    cy.route(
      "POST",
      Cypress.env("VUE_APP_FIREROAD_URL") + "/requirements/progress/minor18/",
      {
    ...
    cy.route(
      "POST",
      Cypress.env("VUE_APP_FIREROAD_URL") + "/requirements/progress/major6-2/",
      {
        "title-no-degree": "Electrical Engineering and Computer Science",
    ...
    cy.getByDataCy("importRoadButton").click();

    cy.get(".v-dialog--active").within(() => {
      const roadFilePath = "../assets/example.road";

      cy.getByDataCy("importRoadFileInput").attachFile(roadFilePath);

      cy.getByDataCy("importRoadTitle").clear().type("My Imported Road");

      cy.getByDataCy("importRoadSubmitButton").click();
    });
    ...
    cy.get("@roadID").then((roadID) => {
      ...
      cy.getByDataCy("road_" + roadID + "__semester_0").within(() => {
        cy.getByDataCy("classInSemester0_8_01").should("exist");
      });

      cy.getByDataCy("road_" + roadID + "__semester_4").within(() => {
        cy.getByDataCy("classInSemester4_6_002").should("exist");
      });
      ...
      cy.getByDataCy("road_" + roadID + "__semester_7").within(() => {
        cy.getByDataCy("classInSemester7_18_100A").should("exist");
      });

      cy.getByDataCy("road_" + roadID + "__semester_15").within(() => {
        cy.getByDataCy("classInSemester15_6_839").should("exist");
      });
    });

    cy.getByDataCy("auditItemgirs").should("exist");

    cy.getByDataCy("auditItemmaster6-P").should("exist");

    cy.getByDataCy("auditItemminor18").should("exist");

    cy.getByDataCy("auditItemmajor6-2").should("exist");
  });
  ...
  it("Exports roads", () => {
    ...
    cy.getByDataCy("road_$defaultroad$__semester_5").click();

    // Add 17.01 to Sophomore IAP
    cy.getByDataCy("classSearchInput").type("17.01");

    cy.dragAndDrop(
      '[data-cy="classInSearch17_01"]',
    ...
    cy.getByDataCy("classSearchInput").clear().type("12.001");

    cy.dragAndDrop(
      '[data-cy="classInSearch12_001"]',
  ...
  it("Loads correct road based on url (Logged Out)", () => {
    ...
    cy.get(".v-dialog--active").within(() => {
      cy.getByDataCy("newRoadName").type("Test Road - Router");
    ...
    cy.get("@roadID").then((roadID) => {
      ...
      cy.getByDataCy(`road_${roadID}__semester_1`).within(() => {
        cy.getByDataCy("classInSemester1_7_46").should("be.visible");
      });
    });
  ...
  it("Loads correct road based on url (Logged In)", () => {
    ...
    cy.getByDataCy(`road_${testRoad.id}__semester_3`).within(() => {
    ...
    cy.getByDataCy(`road_${defaultRoadId}__semester_1`).within(() => {
