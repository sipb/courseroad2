...
describe("Class Info Card Tests", () => {
  it("Opens the class info card", () => {
    ...
    // Search for class
    cy.getByDataCy("classSearchInput").type("10.26");

    // Click on class
    cy.getByDataCy("classInSearch10_26").click();

    // Check that class info card contains the expected information
    cy.getByDataCy("classInfoCard").within(() => {
      // Check for title
      cy.getByDataCy("cardSubjectTitle").should(
        "contain",
      ...
      cy.getByDataCy("cardVirtual").should("contain", "Partly Virtual");

      // Check instructors
      cy.getByDataCy("cardInstructors").should("contain", "G. Rutledge");

      // Check average enrollment
      cy.getByDataCy("cardEnrollment").should("contain", "38");

      // Check average rating
      cy.getByDataCy("cardRating").should("contain", "5.05");
      ...
      cy.getByDataCy("cardDescription").should(
        "contain",
      ...
      cy.getByDataCy("cardPrereqs")
        ...
        .within(() => {
          ...
          cy.getByDataCy("singleScroller10.26prereq0").within(() => {
            cy.getByDataCy("subjectInScroller0").click();
          });

          // Click on first prereq within expansion (the big list)
          cy.getByDataCy("singleScroller10.26prereq0.0").within(() => {
            cy.getByDataCy("subjectInScroller0").click();
          });

          // Click on second prereq within expansion (list of 5.* classes)
          cy.getByDataCy("singleScroller10.26prereq0.0.0").within(() => {
            cy.getByDataCy("subjectInScroller1").click();
          });

          // Check that 5.352 is present on this level
          cy.getByDataCy("singleScroller10.26prereq0.0.0.1").within(() => {
            cy.getByDataCy("subjectInScroller1").should("contain", "5.352");
          });

          // Close the last level (5.* classes)
          cy.getByDataCy("closeButton10.26prereq0.0.0.1").click();

          // Check that it has been closed
          cy.getByDataCy("singleScroller10.26prereq0.0.0.1").should(
            "not.exist",
          );

          // Close all expansions
          cy.getByDataCy("closeButton10.26prereq0.0").click();

          // Check remaining expansions are closed
          cy.getByDataCy("singleScroller10.26prereq0.0.0").should("not.exist");

          cy.getByDataCy("singleScroller10.26prereq0.0").should("not.exist");
        });

      // Check that related subjects are present
      cy.getByDataCy("cardRelatedSubjects").within(() => {
        cy.getByDataCy("subjectInScroller3").should("contain", "6.914");
      });
    });
  });
  ...
  it("Opens class in class info card by clicking in audit", () => {
    ...
    cy.getByDataCy("auditItemgirs.0.4").click();

    // Check that card opened to chemistry GIR
    cy.getByDataCy("classInfoCard").within(() => {
      cy.getByDataCy("cardSubjectTitle").should(
        "contain",
        "Generic Chemistry GIR",
      );
    });
  });
  ...
  it("Opens class in class info card by clicking in road", () => {
    ...
    // Search for class
    cy.getByDataCy("classSearchInput").type("3.010");

    // Drag 3.010 into Freshman Spring
    cy.dragAndDrop(
      '[data-cy="classInSearch3_010"]',
    ...
    cy.getByDataCy("road_$defaultroad$__semester_3").within(() => {
      cy.getByDataCy("classInSemester3_3_010").click();
    });
  ...
  it("Opens class in class info card by clicking in card", () => {
    ...
    cy.getByDataCy("classInfoCard").within(() => {
      ...
      cy.getByDataCy("cardRelatedSubjects").within(() => {
        cy.getByDataCy("subjectInScroller4").click();
      });

      // Check that card opened to 10.10
      cy.getByDataCy("cardSubjectTitle").should("contain", "Molecule Builders");
    });
  });
  ...
  it("Allows the user to go back in class info card history", () => {
    ...
    // Search for class
    cy.getByDataCy("classSearchInput").type("14.21");

    // Drag 14.21 into Freshman Fall
    cy.dragAndDrop(
      '[data-cy="classInSearch14_21"]',
    ...
    cy.getByDataCy("road_$defaultroad$__semester_1").within(() => {
      cy.getByDataCy("classInSemester1_14_21").click();
    });

    // Check that it's in the class info card
    cy.getByDataCy("classInfoCard").within(() => {
      cy.getByDataCy("cardSubjectTitle").should("contain", "Health Economics");
    });

    // Click on class in semester
    cy.getByDataCy("road_$defaultroad$__semester_1").within(() => {
      cy.getByDataCy("classInSemester1_18_100A").click();
    });

    // Check that it's in the class info card
    cy.getByDataCy("classInfoCard").within(() => {
      cy.getByDataCy("cardSubjectTitle").should("contain", "Real Analysis");
    });

    // Click back button
    cy.getByDataCy("cardPreviousButton").click();
