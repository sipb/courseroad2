...
describe("ConflictDialog", () => {
  ...
  it("renders subjects correctly", async () => {
    ...
    wrapper.vm.startConflict();
    await wrapper.vm.$nextTick();
    const cloudSubjects = wrapper
      .find("#selected-subjects-cloud")
      .findAll("span");
    const localSubjects = wrapper
      .find("#selected-subjects-local")
      .findAll("span");
    for (
      let i = 0;
      i < conflictInfo.other_contents.selectedSubjects.length;
      i++
    ) {
      expect(cloudSubjects.at(i).text()).toBe(
        conflictInfo.other_contents.selectedSubjects[i].subject_id,
      );
    }
    for (
      let i = 0;
      i < store.state.roads[45].contents.selectedSubjects.flat().length;
      i++
    ) {
      expect(localSubjects.at(i).text()).toBe(
        store.state.roads[45].contents.selectedSubjects.flat()[i].subject_id,
      );
    }
  });
