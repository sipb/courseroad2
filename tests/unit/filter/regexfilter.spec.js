...
function expectRegexFilterOrder(strings, filter) {
  ...
  for (let i = 0; i < stringRanks.length - 1; i++) {
...
describe("RegexFilter", () => {
  ...
  */
  ...
  it("passes matching subjects with empty requires set up", () => {
    const r = new RegexFilter("test", "t", "n[^n]{2,3}n", undefined, ["a"]);
  ...
  it("passes matching subject with empty requires set up with invalid regex", () => {
    const r = new RegexFilter("test", "t", "ab", undefined, ["a"]);
    r.setupInputs({ in: "[" });
  ...
  it("passes matching subjects with inputs set up", () => {
    const r = new RegexFilter("test", "t", "20\\d\\d-05-", "day", ["a"], "OR");
  ...
  it("passes matching subject with inputs set up using invalid regex", () => {
    const r = new RegexFilter("test", "t", "h.t", "hi", ["a"], "OR");
    r.setupInputs({ hi: "\\" });
  ...
  it("fails non-matching subjects with inputs set up", () => {
    const r = new RegexFilter("test", "t", "i like \\d+ ", "pets", ["a"], "OR");
  ...
  it("fails non-matching subjects with inputs set up using invalid regex", () => {
    const r = new RegexFilter("test", "r", "ch.+s\\s+", "x", ["a"], "OR");
    r.setupInputs({ x: "+8" });
  ...
  it("ranks subjects correctly with 0 priorities", () => {
    const r = new RegexFilter("test", "t", "s.{3}e", undefined, ["a"]);
    r.setupVariants({ x: "y" }, {}, []);
  ...
  it("ranks subjects correctly for positive priorities", () => {
    ...
    r.setupVariants({}, { atStart: true, asLiteral: true }, [
      "asLiteral",
      "atStart",
    ]);
    expectRegexFilterOrder(
      ["sun", "afternoon", "noon", "abc(.)(.)\\2\\1", "(.)(.)\\2\\1xyz"],
      r,
    );
  });
  ...
  it("ranks subjects correctly for mixed priorities", () => {
    ...
    r.setupVariants({}, { atStart: true, asLiteral: false }, [
      "atStart",
      "asLiteral",
    ]);
    expectRegexFilterOrder(
      ["16.400", "12.742", "17.270", "2.00B", "21G.701"],
      r,
    );
  });
  ...
  it("ranks subjects correctly for negative priorities", () => {
    ...
    r.setupVariants({}, { atStart: false, asLiteral: false }, [
      "asLiteral",
      "atStart",
    ]);
  ...
  it("ranks subjects correctly for 0 priorities with inputs", () => {
    const r = new RegexFilter("test", "t", "a.b.c.", "b", ["a"], "OR");
    r.setupVariants({ b: "d" }, {}, []);
  ...
  it("ranks subjects correctly for positive priorities with inputs", () => {
    ...
    r.setupVariants({ a: "at" }, { atStart: true, asLiteral: true }, [
      "atStart",
      "asLiteral",
    ]);
  ...
  it("ranks subjects correctly for mixed priorities with inputs", () => {
    ...
    r.setupVariants({ y: ".*s" }, { atStart: false, asLiteral: true }, [
      "asLiteral",
      "atStart",
    ]);
    expectRegexFilterOrder(
      ["acorn", "paths", "apartments", "p.+t.*s", "all p.+t.*s"],
      r,
    );
  });
  ...
  it("ranks subjects correctly for negative priorities with inputs", () => {
    ...
    r.setupVariants({ in: "r" }, { atStart: false, asLiteral: false }, [
      "atStart",
      "asLiteral",
    ]);
    expectRegexFilterOrder(
      ["blank", "e..er :(", "experts", "fore..er", "centers"],
      r,
    );
  });
});
