...
describe('Filters', () => {
  ...
  */
  ...
  it('lets no subjects match a zero-attribute OR filter', () => {
    let f = new Filter("test", "t", ((x) => x === 7), [], 'OR');
    expect(f.matches({y: 8})).toBe(false);
  });

  // covers this.attributeNames.length = 0, 'AND', matches, 0 failing attributes
  it('lets all subjects match a zero-attribute AND filter', () => {
    let f = new Filter("test", "t", ((x) => x === 7), [], 'AND');
    expect(f.matches({x: 8, y: 7, z: 9})).toBe(true);
  });

  // covers this.attributeNames.length = 1, 'OR', doesn't match, all failing attributes
  it('fails non-matching subjects against a one-attribute OR filter', () => {
    let f = new Filter("test", "t", ((x) => x === 7), ['x'], 'OR');
    expect(f.matches({x: 20, y: 4})).toBe(false);
  });

  // covers this.attributeNames.length = 1, 'OR', matches, 0 failing attributes
  it('passes matching subjects against a one-attribute OR filter', () => {
    let f = new Filter("test", "t", ((x) => x === 7), ['y'], 'OR');
    expect(f.matches({x: 10, y: 7})).toBe(true);
  });

  // covers this.attributeNames.length = 1, 'AND', doesn't match, all failing attributes
  it('fails non-matching subjects against a one-attribute AND filter', () => {
    let f = new Filter("test", "t", ((x) => x === 7), ['z'], 'AND');
    expect(f.matches({x: 'hi', z: 8})).toBe(false);
  });

  // covers this.attributeNames.length = 1, 'AND', matches, 0 failing attributes
  it('passes matching subjects against a one-attribute AND filter', () => {
    let f = new Filter("test", "t", ((x) => x === 7), ['x'], 'AND');
    expect(f.matches({x: 7, y: 9})).toBe(true);
  });

  // covers this.attributeNames.length > 1, 'OR', doesn't match, all failing attributes
  it('fails non-matching subjects against a multi-attribute OR filter', () => {
    let f = new Filter("test", "t", ((x) => x === 7), ['x', 'y', 'z'], 'OR');
    expect(f.matches({x: 8, y: 10, z: 6, u: 20})).toBe(false);
  });

  // covers this.attributeNames.length > 1, 'OR', matches, some failing attributes
  it('passes semi-matching subjects against a multi-attribute OR filter', () => {
    let f = new Filter("test", "t", ((x) => x === 7), ['x', 'y', 'z', 'w'], 'OR');
    expect(f.matches({x: 8, y: 7, z: 10, w: 6})).toBe(true);
  });

  // covers this.attributeNames.length > 1, 'OR', matches, 0 failing attributes
  it("passes matching subjects against a multi-attribute OR filter", () => {
    let f = new Filter("test", "t", ((x) => x === 7), ['a', 'b', 'c'], 'OR');
    expect(f.matches({a: 7, b: 7, c: 7, d: 12})).toBe(true);
  });

  // covers this.attributeNames.length > 1, 'AND', doesn't match, all failing attributes
  it("fails non-matching subjects against a multi-attribute AND filter", () => {
    let f = new Filter("test", "t", ((x) => x === 7), ['x', 'y', 'z'], 'AND');
  ...
  it("fails semi-matching subjects against a multi-attribute AND filter", () => {
    let f = new Filter("test", "t", ((x) => x === 7), ['a', 'b', 'c', 'd'], 'AND');
    expect(f.matches({a: 7, b: 6, c: 7, d: 8})).toBe(false);
  });

  // covers this.attributeNames.length > 1, 'AND', matches, 0 failing attributes
  it("passes matching subjects against a multi-attribute AND filter", () => {
    let f = new Filter("test", "t", ((x) => x === 7), ['p', 'q', 'r', 's'], 'AND');
