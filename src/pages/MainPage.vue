...
export default {
  ...
  methods: {
    ...
    updateFulfillment: function() {
      for (var r = 0; r < this.roads[this.activeRoad].contents.coursesOfStudy.length; r++) {
        ...
        axios.post(FIREROAD_LINK + `/requirements/progress/`+req+`/`,this.roads[this.activeRoad].contents).then(function(response) {
  ...
  mounted() {
    ...
    axios.get(FIREROAD_LINK + `/requirements/list_reqs/`)
      .then(response => {
        const ordered = {};
    ...
    axios.get(FIREROAD_LINK + `/courses/all?full=true`)
      .then(response => {
