...
export default {
  ...
  computed: {
    ...
    saveIcon: function () {
      return this.saveWarnings.length ? "mdi-alert" : "mdi-content-save";
    },
  },
  watch: {
  ...
  methods: {
        ...
        .then(
          function (verifyResponse) {
            if (verifyResponse.data.success) {
              this.$store.commit(
                "setCurrentSemester",
                verifyResponse.data.current_semester -
                  (currentMonth === 4 ? 1 : 0),
              );
    ...
    retrieveRoad: function (roadID) {
      ...
      return this.getSecure("/sync/roads/?id=" + roadID).then(
        function (roadData) {
    ...
    sanitizeRoad: function (road) {
      ...
      road.contents.selectedSubjects = this.getSimpleSelectedSubjects(
        ...
        .then(
          function (files) {
            ...
            if (fileKeys.includes(this.$route.params.road)) {
              ...
              this.$router.push({
                path: `/road/${Object.keys(this.roads)[0]}`,
              });
            }
    ...
    attemptLogin: function () {
      ...
      if ("code" in queryObject) {
        ...
        this.$cookies.get("hasLoggedIn") === "true" &&
        !this.loggedIn
      ) {
        this.loginUser();
      }
    ...
    saveRemote: function (roadID, override) {
      ...
      this.saveWarnings = [];
      const assignKeys = { override, agent: this.getAgent() };
      if (!roadID.includes("$")) {
        assignKeys.id = roadID;
      }
      const roadSubjects = this.flatten(
        this.roads[roadID].contents.selectedSubjects,
      );
      ...
      const savePromise = this.postSecure("/sync/sync_road/", roadToSend).then(
        function (response) {
          if (response.status !== 200) {
            return Promise.reject(
              new Error("Unable to save road " + this.oldid),
            );
          } else {
            const newid =
              response.data.id !== undefined ? response.data.id : this.oldid;
            if (response.data.success === false) {
              this.data.saveWarnings.push({
                id: newid,
                error: response.data.error_msg,
                name: this.data.roads[this.oldid].name,
              });
            }
            if (response.data.result === "conflict") {
              ...
              if (response.data.id !== undefined) {
                ...
                if (this.oldid !== response.data.id.toString()) {
                  this.data.$store.commit("resetID", {
                    oldid: this.oldid,
                    newid: response.data.id,
                  });
                }
    ...
    getNewRoadData: function () {
      const newRoadData = {};
      if (
        this.newRoads.indexOf("$defaultroad$") === -1 &&
        "$defaultroad$" in this.roads
      ) {
        if (
          this.flatten(this.roads.$defaultroad$.contents.selectedSubjects)
            .length > 0 ||
          JSON.stringify(
            Array.from(this.roads.$defaultroad$.contents.coursesOfStudy),
          ) !== '["girs"]'
        ) {
    ...
    deleteRoad: function (roadID) {
      if (this.activeRoad === roadID) {
        ...
        const withoutRoad = Object.keys(this.roads)
          .slice(0, roadIndex)
          .concat(Object.keys(this.roads).slice(roadIndex + 1));
