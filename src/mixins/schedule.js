  ...
  methods: {
    lateSchedule: function (subj, genericIndex, compareDate = new Date()) {
      if (subj.schedule === undefined && !(subj.subject_id in genericIndex)) {
        ...
        const lateForFall = scheduleForFall && subj.offered_fall && compareDate > fallCutoff;
        const lateForSpring = !scheduleForFall && subj.offered_spring && compareDate > springCutoff;
